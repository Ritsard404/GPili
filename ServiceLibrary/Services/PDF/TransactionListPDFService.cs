...
namespace EBISX_POS.API.Services.PDF
{
    public class TransactionListPDFService
    {
        ...
        public byte[] GeneratePwdOrSeniorListPDF(List<TransactionListDTO> transactions, DateTime fromDate, DateTime toDate, string type)
        {
            ...
            var title = type.ToUpper() == DiscountType.Pwd ? "PERSON WITH DISABILITY LIST" : "SENIOR CITIZEN LIST";
            ...
            gfx.DrawString($"TIN {_tin}", normalFont, XBrushes.Black, new XPoint(margin, y));
            y += 18;
            gfx.DrawString(title, headerFont, XBrushes.DarkBlue, new XPoint(margin, y));
            y += 14;
            gfx.DrawString($"From {fromDate:MM-dd-yyyy} To {toDate:MM-dd-yyyy}", normalFont, XBrushes.Black, new XPoint(margin, y));
            y += 18;
            tableTop = y;
